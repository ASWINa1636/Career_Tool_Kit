#----------------------------THIS MAIN.PY CODE WILL BE STORE IN THE (MYSQL DATABASE) "ADS.sql"-----------------------------------
...
def ats_scanner():
    while True:
        ...
        print("2. View Resume Scan History")
        ...
        if sub_choice == '1':
            ...
            cursor.execute(
                "INSERT INTO resume_scans (file_path, score) VALUES (%s, %s)",
                (file_path, score)
            )
            conn.commit()
            ...
            cursor.execute(
                ...
                (f"Scanned resume: {file_path} - Score: {score}",)
            )
            conn.commit()
            ...
            print("\n--- Resume Scan History ---")
            cursor.execute("SELECT file_path, score, scan_time FROM resume_scans ORDER BY scan_time DESC")
            rows = cursor.fetchall()

            if not rows:
                print("No resume scans found.")
            else:
                for idx, row in enumerate(rows, 1):
                    print(f"{idx}. {row[0]} - {row[1]}/100 - {row[2].strftime('%d/%m/%Y %I:%M %p')}")
...
def cgpa_calculator():
    ...
    try:
        ...
        # Save result in MySQL history table
        gpa_string = ', '.join(map(str, gpas))  # Convert list to string

        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute(
            "INSERT INTO cgpa_history (semester, gpa, final_cgpa, entry_time) VALUES (%s, %s, %s, %s)",
            (total_semesters, gpa_string, f"{cgpa:.2f}", timestamp)
        )
        conn.commit()

        print("📝 CGPA saved in history.")
        input("Press Enter to continue...")
...
def view_history():
    ...
    try:
        ...
        if not rows:
            ...
            for idx, row in enumerate(rows, 1):
                print(f"{idx}. {row[0]} - Score: {row[1]}/100 - {row[2].strftime('%d/%m/%Y %I:%M %p')}")

        # View CGPA Calculation History
        print("\n--- CGPA History ---")
        cursor.execute("SELECT semester, gpa, final_cgpa, entry_time FROM cgpa_history ORDER BY entry_time DESC")
        rows = cursor.fetchall()
        if not rows:
            print("No CGPA records found.")
        else:
            for idx, row in enumerate(rows, 1):
                print(f"{idx}. Semesters: {row[0]} | GPA(s): {row[1]} | Final CGPA: {row[2]} | Time: {row[3].strftime('%d/%m/%Y %I:%M %p')}")
